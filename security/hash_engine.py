# security/hash_engine.py

def hash_password(password):
    if not password:
        return ""
    
    salt = "static_lab_salt_123"
    message = str(password) + salt
    
    # --- 1. Pre-processing (Padding) ---
    # Convert message to bit array
    data = bytearray(message, 'utf-8')
    orig_len_bits = (len(data) * 8) & 0xffffffffffffffff
    
    # Append '1' bit (0x80 byte)
    data.append(0x80)
    
    # Append '0' bits until length is 448 (mod 512)
    while (len(data) * 8) % 512 != 448:
        data.append(0x00)
    
    # Append original length as 64-bit big-endian integer
    data += orig_len_bits.to_bytes(8, byteorder='big')

    # --- 2. Initialize Hash Values (First 32 bits of fractional parts of square roots of first 8 primes) ---
    h = [
        0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
        0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
    ]

    # --- 3. Constants (First 32 bits of fractional parts of cube roots of first 64 primes) ---
    k = [
        0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
        0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
        0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
        0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
        0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
        0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
        0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
        0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
    ]

    # Helper functions for bitwise operations
    def rotr(x, n): return ((x >> n) | (x << (32 - n))) & 0xffffffff
    
    # --- 4. Main Loop ---
    for i in range(0, len(data), 64):
        chunk = data[i:i+64]
        w = [0] * 64
        
        # Break chunk into sixteen 32-bit big-endian words
        for t in range(16):
            w[t] = int.from_bytes(chunk[t*4:t*4+4], byteorder='big')
            
        # Extend the first 16 words into the remaining 48 words
        for t in range(16, 64):
            s0 = rotr(w[t-15], 7) ^ rotr(w[t-15], 18) ^ (w[t-15] >> 3)
            s1 = rotr(w[t-2], 17) ^ rotr(w[t-2], 19) ^ (w[t-2] >> 10)
            w[t] = (w[t-16] + s0 + w[t-7] + s1) & 0xffffffff

        # Initialize working variables
        a, b, c, d, e, f, g, h_var = h

        # Compression loop
        for t in range(64):
            S1 = rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25)
            ch = (e & f) ^ ((~e) & g)
            temp1 = (h_var + S1 + ch + k[t] + w[t]) & 0xffffffff
            S0 = rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22)
            maj = (a & b) ^ (a & c) ^ (b & c)
            temp2 = (S0 + maj) & 0xffffffff

            h_var = g
            g = f
            f = e
            e = (d + temp1) & 0xffffffff
            d = c
            c = b
            b = a
            a = (temp1 + temp2) & 0xffffffff

        # Add compressed chunk to current hash value
        vals = [a, b, c, d, e, f, g, h_var]
        for j in range(8):
            h[j] = (h[j] + vals[j]) & 0xffffffff

    # Produce the final hash (big-endian) as a hex string
    return ''.join(format(x, '08x') for x in h)